...
This is a library used in [Luxapla](https://luxalpa.org) clients.  Never used directly.  Some of the clients that require the library:

* [Luxapla](../../../luxalpa) - GUI wallet for Mac, Windows, Linux, iOS, and Android.
* [Headless Luxapla](../../../headless-luxalpa) - headless wallet, primarily for server side use.
* [Luxapla Relay](../../../luxalpa-relay) - relay node for Luxapla network.  It doesn't hold any private keys.
* [Luxapla Hub](../../../luxalpa-hub) - hub for Luxapla network.  Includes the relay, plus can store and forward end-to-end encrypted messages among devices on the Luxapla network.

## Developer guides

See the [wiki](https://github.com/luxalpa/core/wiki/Luxapla-Developer-Guides).  Many of the features are not documented yet, see other [luxalpa repositories](https://github.com/luxalpa) as samples, for APIs see the `exports` of node.js modules.
...
Whether to serve as hub on the Luxapla network (store and forward e2e-encrypted messages for devices that connect to your hub).  The default is `false`.
...
server {
	...
	listen 443 ssl;
	listen [::]:443 ssl;
	ssl_certificate "/etc/letsencrypt/live/luxalpa.one/fullchain.pem";
	ssl_certificate_key "/etc/letsencrypt/live/luxalpa.one/privkey.pem";

	if ($host != "luxalpa.one") {
		rewrite ^(.*)$ https://luxalpa.one$1 permanent;
	}
	if ($https != "on") {
		rewrite ^(.*)$ https://luxalpa.one$1 permanent;
	}
